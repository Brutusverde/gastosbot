...
async def gasto(update, context, group_id = None):
    ...
    if len(gasto_usuario) < 2:
        await update.effective_message.reply_text("⚠️ Uso correcto: /gasto [cantidad] [descripción] [categoría opcional]")
        return
    ...
    await update.effective_message.reply_text(f"✅ Gasto de *{gasto_usuario[0]}€* en *{gasto_usuario[1]}* registrado correctamente", parse_mode='Markdown')
...
async def deudas(update, context, group_id = None):
    ...
    if len(deudas) == 0:
        await update.effective_message.reply_text("✅ No hay deudas pendientes en el grupo")
    else:
...
async def misdeudas(update, context, group_id = None):
    ...
    if len(misdeudas) == 0:
        await update.effective_message.reply_text("✅ No tienes deudas pendientes")
    else:
...
async def historial(update, context, group_id = None):
    ...
    if len(gastos) == 0:
        await update.effective_message.reply_text("📋 Aún no hay ningún gasto registrado")
    else:
...
async def saldar(update, context, group_id = None):
    ...
    if len(context.args) < 1:
        await update.effective_message.reply_text("⚠️ Uso correcto: /saldar [@usuario]")
        return
...
async def handler_crear_grupo(update, context):
    ...
    if len(context.args) < 1:
        await update.effective_message.reply_text("⚠️ Uso correcto: /crear_grupo [nombre]")
        return
...
async def handler_unirse (update, context):
    ...
    if grupo_unirse is None:
        await update.effective_message.reply_text(f"El grupo al que intentas unirte no existe")
        return

    usuario_en_grupo = obtener_usuario_en_grupo(id, grupo_unirse[0])

    if usuario_en_grupo is not None:
        await update.effective_message.reply_text(f"Ya perteneces a este grupo")
        return
...
async def misgrupos(update, context):
    ...
    if len(grupos) == 0:
        await update.effective_message.reply_text("⚠️ No perteneces a ningun grupo")
    else:
        mensaje = "👥 *Grupos en los que participas:*\n\n"
...
async def mostrar_selector_grupo(update, context, comando):
    ...
    await update.effective_message.reply_text("¿En qué grupo quieres ejecutar este comando?", reply_markup=teclado)
